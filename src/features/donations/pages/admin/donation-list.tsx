...
export default function DonationListPage() {
  ...
  const safeDonations = useMemo(() => {
    ...
    if (donorFilter) {
      items = items.filter((d) => {
        ...
        return (
          name.includes(donorFilter.toLowerCase()) ||
          d.donorId.toString().includes(donorFilter)
        );
      });
    }
    if (hospitalFilter) {
      items = items.filter((d) => {
        ...
        return (
          name.includes(hospitalFilter.toLowerCase()) ||
          d.hospitalId.toString().includes(hospitalFilter)
        );
      });
    }
    return items;
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: donationKeys.lists() });
      toast.success(`Donation status updated to ${data.status}`, {
  ...
  return (
    <Card className="px-8 py-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-4 bg-secondary/10 rounded-lg">
        ...
        <div className="flex-1">
          <Label htmlFor="donorFilter" className="text-xs mb-1 block">
            Donor
          </Label>
          <Input
            id="donorFilter"
        ...
        <div className="flex-1">
          <Label htmlFor="hospitalFilter" className="text-xs mb-1 block">
            Hospital
          </Label>
          <Input
            id="hospitalFilter"
